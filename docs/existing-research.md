...
| HTTPX QuickStart | https://www.python-httpx.org/quickstart/ | Official HTTPX docs | Confirms binary and `iter_bytes()` responses automatically decode HTTP gzip and deflate content coding, while `iter_raw()` does not. | Referenced to require identity encoding for file-download byte accounting | 2026-08-31 |
