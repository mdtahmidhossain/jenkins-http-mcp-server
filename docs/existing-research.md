...
| HTTPX QuickStart | https://www.python-httpx.org/quickstart/ | Official HTTPX docs | Confirms binary and `iter_bytes()` responses automatically decode HTTP gzip and deflate content coding, while `iter_raw()` does not. | Referenced for decoded-response metadata normalization and identity encoding for file downloads | 2026-08-31 |
